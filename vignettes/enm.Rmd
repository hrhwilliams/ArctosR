...
turkey tick) and use their geographical coordinate data to model their
distribution in North America. This vignette closely follows the usage example
from <https://github.com/marlonecobos/kuenm> but uses data downloaded directly
from Arctos via ArctosR.
...
those names are what the `kuenm2` package expects.
...
Next, we use the `geodata` package to download climate data from WorldClim, which
will form part of the input into the models we are going to train. It will save
these data as files, so we pass it our current working directory as the path to
save those files to.
...
Similarly we filter the WorldClim data to only coordinates in North America.
...
Now, we use `kuenm2`'s built-in data cleaning functions to prepare the data for
...
Now we pass the cleaned data to `kuenm2` to train and evaluate candidate models.
